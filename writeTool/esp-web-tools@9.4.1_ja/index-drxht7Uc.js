...
`; let r = class extends l {
  render() {
    return i`
      <ewt-dialog
        open
        heading="ポートが選択されていません！"
        scrimClickAction
        @closed=${this._handleClose}
      >
        <div>
          お使いのデバイスがリストになくポートを選択できなかった場合は，次の手順をお試しください．
        </div>
        <ol>
          <li>
            このWebサイトを表示しているコンピュータに，デバイスが接続されているか確認してください．
          </li>
          <li>
            コンピュータにデバイスを接続した際に，デバイスの電源が入っていることを確認してください．
          </li>
          <li>
            使用しているUSBケーブルが，データ転送用のケーブルであることを確認してください．
          </li>
          <li>
            正しいドライバがインストールされているか確認してください．インストールされていない場合は，以下のURLからインストールしてください．
            <ul>
              <li>
                CP2102 ドライバ:
                <a
    ...
    `} async _handleClose() { this.parentNode.removeChild(this) }
}; r.styles = [e, t`
      li + li,
      li > ul {
        margin-top: 8px;
